...
export default async function (req) {
  ...
  const servicePath = pathParts.slice(2).join('/') + url.search
  ...
  if (!baseUrl)
    return new Response(JSON.stringify({ error: 'No mapping found', serviceKey }), { status: 502, headers: { 'content-type': 'application/json' } })

  const targetUrl = `${baseUrl.replace(/\/$/, '')}/${servicePath}`
  ...
  try {
    ...
    return new Response(respBody === null ? null : respBody, { status: res.status, statusText: res.statusText, headers: respHeaders })
  } catch (err) {
    return new Response(JSON.stringify({ error: 'Upstream request failed', details: String(err) }), { status: 502, headers: { 'content-type': 'application/json' } })
  }
}
